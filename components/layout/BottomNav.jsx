        ...
        <>
          ...
          <section className="app-bottom-sheet" role="dialog" aria-modal="true" aria-label={isFrench ? "Plus d’options" : "المزيد من الخيارات"}>
            ...
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                <p className="text-[10px] font-black text-mauri-green dark:text-emerald-300">MauriResults</p>
                <h2 className="mt-1 text-xl font-black text-slate-950 dark:text-white">{isFrench ? "Plus de services" : "خدمات إضافية"}</h2>
              </div>
            ...
            <div className="grid grid-cols-2 gap-2.5">
              {moreItems.map((item) => (
                ...
                >
                  ...
                  <span>
                    <strong className="block text-sm font-black text-slate-950 dark:text-white">{item.label}</strong>
                    <small className="mt-1 block text-[10px] font-bold leading-4 text-slate-500 dark:text-slate-400">{item.description}</small>
                  </span>
                </button>
              ))}
            </div>
