...
begin
  ...
  execute format('select coalesce(jsonb_agg(to_jsonb(t)),''[]''::jsonb) from (select %1$I::text id,%2$I::text name,%3$s "MOD",%4$s track,%5$s kr,%6$s wl,%7$s moughataa,%8$s ms,%9$s centre,%10$s "birthPlace",%11$s "birthDate",%12$s rank,%13$L source,%14$L "sessionType" from public.%15$I order by %16$s limit 100)t', exam.number_column, exam.name_column, score_expr, track_expr, decision_expr, wilaya_expr, moughataa_expr, school_expr, centre_expr, birth_place_expr, birth_date_expr, rank_expr, p_source_key, exam.title_ar, relation_name, order_expr) into top_students;
