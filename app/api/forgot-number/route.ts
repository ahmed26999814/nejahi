...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Candidate lists must be narrowed before querying. This prevents a direct
    // request from returning a large portion of an exam table.
    if (!centre && !school && name.length < 2) {
      return NextResponse.json(
    ...
    return NextResponse.json({ candidates: Array.isArray(rows) ? rows.slice(0, 250) : [] }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
