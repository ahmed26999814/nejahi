...
function normalizeStats(row: Record<string, unknown> | undefined, source: string) {
  ...
  return { ...(row || {}), total, passed, failed: isConcours ? 0 : numberValue(row?.failed), average: numberValue(row?.average ?? row?.average_score), highest: numberValue(row?.highest ?? row?.highest_score), passRate: total ? (passed / total) * 100 : 0, isConcours };
}
...
function normalizeBreakdown(rows: Array<Record<string, unknown>> = [], source: string) {
  ...
  return rows.map((row) => {
    const total = numberValue(row.total ?? row.total_students);
    const passed = isConcours ? 0 : numberValue(row.passed);
    return { ...row, label: labelValue(row), total, passed, average: numberValue(row.average ?? row.average_score), highest: numberValue(row.highest ?? row.highest_score), passRate: total ? (passed / total) * 100 : 0 };
  });
}
...
async function supabaseJson(url: URL, init?: RequestInit) {
  ...
  const response = await fetch(url, {
    ...
    headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}`, Accept: "application/json", ...(init?.headers || {}) },
    cache: "no-store",
  });
...
async function getUploadedDashboard(source: string) {
  ...
  return (await supabaseJson(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ p_source_key: source }) })) || {};
}
...
async function getLegacyDashboard(source: string) {
  ...
  const [statsRows, regionRows, schoolRows, trackRows, moughataaRows, topRows] = await Promise.all([
    fetchLegacyView(views.stats, 1), fetchLegacyView(views.regionStats), fetchLegacyView(views.schoolStats),
    fetchLegacyView(views.trackStats), fetchLegacyView(views.moughataaStats), fetchLegacyView(views.topStudents),
  ]);
  return {
    stats: normalizeStats(statsRows?.[0], source), regionStats: normalizeBreakdown(regionRows, source),
    schoolStats: normalizeBreakdown(schoolRows, source), trackStats: source === "concours" || source === "brevet" ? [] : normalizeBreakdown(trackRows, source),
    moughataaStats: normalizeBreakdown(moughataaRows, source), topStudents: normalizeTopStudents(topRows),
  };
}
...
function loadDashboard(source: string) {
  ...
  const request = (source.startsWith("upload:") ? getUploadedDashboard(source) : getLegacyDashboard(source))
    .then((data) => { memoryCache.set(source, { data, createdAt: Date.now() }); return data; })
...
export async function GET(request: Request) {
  ...
  try { return json(await loadDashboard(source), "MISS"); }
  catch (error) {
    if (cached) return json(cached.data, "STALE-IF-ERROR");
    return NextResponse.json({ error: "تعذر تحميل الإحصائيات مؤقتًا" }, { status: 503, headers: { "Cache-Control": "public, s-maxage=30, stale-if-error=300", "Retry-After": "30" } });
  }
}
