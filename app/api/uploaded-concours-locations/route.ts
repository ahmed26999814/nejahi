...
async function directConcoursOptions(level: string, wilaya: string, moughataa: string) {
  if (!SUPABASE_URL || !SUPABASE_KEY) return { options: [], status: 500, error: "Missing Supabase environment variables" };
  const column = level === "wilaya" ? "WILAYA_AR" : level === "moughataa" ? "MOUGHATAA_AR" : "Centre Examen_AR";
  const url = new URL(`${SUPABASE_URL}/rest/v1/concours_results`);
  url.searchParams.set("select", `\"${column}\"`);
  url.searchParams.set(`\"${column}\"`, "not.is.null");
  if (level !== "wilaya" && wilaya) url.searchParams.set("WILAYA_AR", `eq.${wilaya}`);
  if (level === "centre" && moughataa) url.searchParams.set("MOUGHATAA_AR", `eq.${moughataa}`);
  url.searchParams.set("order", `\"${column}\".asc`);
  url.searchParams.set("limit", "5000");
  const response = await fetch(url, { cache: "no-store", headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}`, Accept: "application/json", Prefer: "count=none" } });
  const text = await response.text();
  if (!response.ok) return { options: [], status: response.status, error: text.slice(0, 900) };
  const rows = text ? JSON.parse(text) : [];
  const options = Array.from(new Set(rows.map((row: Record<string, unknown>) => clean(row[column])).filter(Boolean)));
...
export async function GET(request: Request) {
  ...
  if (source === "concours_2026" || source === "concours") {
    const result = await directConcoursOptions(level, wilaya, moughataa);
    return NextResponse.json({ options: result.options, ...(result.error ? { error: result.error } : {}) }, { status: result.status, headers: { "Cache-Control": result.status === 200 ? "public, s-maxage=3600, stale-while-revalidate=86400" : "no-store" } });
  }
